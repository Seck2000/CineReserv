    ...
    public class ApiService : IApiService
    {
        ...
        public async Task SeedDatabaseAsync()
        {
            try
            {
                ...
                // Créer les 200 sièges fixes pour chaque salle
                foreach (var salle in salles)
                {
                    await CreateSiegesForSalle(salle.Id, 200);
                    _logger.LogInformation($"200 sièges créés pour {salle.Nom}");
                }
        ...
        public async Task UpdateAllSallesTo200PlacesAsync()
        {
            try
            {
                ...
                foreach (var salle in salles)
                {
                    ...
                    var siegesReserves = siegesExistants.Count(s => s.EstReserve);
                    
                    // Supprimer uniquement les sièges non réservés
                    var siegesNonReserves = siegesExistants.Where(s => !s.EstReserve).ToList();
